...
class AmpliconSequenceFileAdmin(SequenceFileAdmin):
    # removed run
    fieldsets = [
        ('Sequence File', {'fields': ('filename', 'md5', 'sample', 'lane_number', 'index_number', 'analysed',
...
class AmpliconMetadaAdmin(admin.ModelAdmin):
        ...
        class Meta:
            ...
            widgets = {
                'bpa_id': LinkedSelect(attrs={'class': 'input-medium',
                                              'style': 'width:50%'}),
                'sample_extraction_id': forms.TextInput(attrs={'class': 'input-medium',
                                                               'style': 'width:50%'}),
                'debug_note': AutosizedTextarea(attrs={'class': 'input-large',
                                                       'style': 'width:95%'}),
                'comments': AutosizedTextarea(attrs={'class': 'input-large',
                                                     'style': 'width:95%'}),
    ...
    suit_form_tabs = (('id', 'Sample ID and Sequence Files'),
                      ('management',
                       'Sample Management', ),
    ...
    fieldsets = [
        (None,  # 'Sample Identification',
         {'classes': ('suit-tab suit-tab-id', ),
          'fields': ('bpa_id', 'sample_extraction_id')}),
        (None,  # 'Sample Management',
         {'classes': ('suit-tab suit-tab-management', ),
          'fields': ('sequencing_facility', 'target', 'index', 'reads', 'pcr_1_to_10', 'pcr_1_to_100', 'pcr_neat',
                     'dilution', 'analysis_software_version')}),
        (None,  # 'Notes',
         {'classes': ('suit-tab suit-tab-notes', ),
          'fields': ('debug_note', 'comments')}),
    ]
