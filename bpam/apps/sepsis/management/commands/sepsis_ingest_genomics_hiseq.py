...
from ...models import GenomicsHiseqFile, SepsisSample, HiseqGenomicsMethod
...
METADATA_PATH = "sepsis/genomics/hiseq/"
...
def add_method_data(data):
    ...
    for entry in data:
        ...
        method, _ = HiseqGenomicsMethod.objects.get_or_create(**args)
        if method:
            files = GenomicsHiseqFile.objects.filter(sample__bpa_id=bpa_id)
            for f in files:
                f.method = method
                f.save()
...
def do_metadata():
    ...
    logger.info("Ingesting Sepsis Genomics Hiseq metadata from {0}".format(DATA_DIR))
...
def add_md5(md5_lines):
    ...
    for md5_line in md5_lines:
        ...
        # add files
        f, _ = GenomicsHiseqFile.objects.get_or_create(note="Ingested using management command",
                                                       sample=sample,
...
class Command(management_command.BPACommand):
    help = 'Ingest Sepsis Genomics hiseq metadata'
    ...
    def handle(self, *args, **options):
        ...
        if options['delete']:
            ...
            GenomicsHiseqFile.objects.all().delete()
