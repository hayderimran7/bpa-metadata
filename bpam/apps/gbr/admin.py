...
class ProtocolForm(forms.ModelForm):
    class Meta:
        ...
        model = GBRProtocol
        widgets = {
            'library_construction_protocol': forms.TextInput(attrs={'class': 'input-large',
                                                                    'style': 'width:95%'}),
...
class SampleAdmin(admin.ModelAdmin):
    class SampleForm(forms.ModelForm):
        class Meta:
            ...
            widgets = {
                ...
                'comments_by_facility': AutosizedTextarea(attrs={'class': 'input-large',
                                                                 'style': 'width:95%'}),
                'sequencing_notes': AutosizedTextarea(attrs={'class': 'input-large',
                                                             'style': 'width:95%'}),
                'note': AutosizedTextarea(attrs={'class': 'input-large',
                                                 'style': 'width:95%'}),
    ...
    suit_form_tabs = (('id', 'Sample ID and Sequence Files'), ('dna', 'DNA'), ('management',
                                                                               'Sample Management', ), ('contacts',
                                                                                                        'Contacts', ),
                      ('notes', 'Source Data Note'))

    fieldsets = [
        ('ID', {'classes': ('suit-tab suit-tab-id', ),
                'fields': ('bpa_id',
                           'name', )}),
        (None,  # 'DNA/RNA Source',
        ...
        (None,  # 'Sample Management',
         {'classes': ('suit-tab suit-tab-management', ),
          'fields':
          ('dataset', 'requested_sequence_coverage', 'requested_read_length', 'date_sent_to_sequencing_facility',
           'date_sequenced', 'sequencing_data_eta', 'comments_by_facility', 'sequencing_notes')}),
        (None,  # 'Contacts',
...
class CollectionEventAdmin(admin.ModelAdmin):
    ...
    form = CollectionForm
    fieldsets = [
        ('Collection', {'fields': ('collection_date',
                                   'collector', ), }),
        ('Site Data', {'fields': ('water_temp', 'water_ph', 'depth')}),
        ('Note', {'fields': ('note', )}),
    ]
...
class SiteAdmin(admin.ModelAdmin):
    class CollectionForm(forms.ModelForm):
        class Meta:
            ...
            widgets = {
                'site_name': AutosizedTextarea(attrs={'class': 'input-large',
                                                      'style': 'width:95%'}),
    ...
    form = CollectionForm
    fieldsets = [
        ('Collection', {'fields': ('site_name', 'lat', 'lon'), }),
        ('Note', {'fields': ('note', )}),
    ]
