...
class SampleContextAdmin(admin.ModelAdmin):
        ...
        class Meta:
            ...
            widgets = {
                ...
                'debug_note': AutosizedTextarea(attrs={'class': 'input-large',
                                                       'style': 'width:95%'}),
                'methodological_notes': AutosizedTextarea(attrs={'class': 'input-large',
                                                                 'style': 'width:95%'}),
            }
    ...
    fieldsets = [
        ('Sample Description', {'description': 'Sample contextual information',
                                'fields': ('bpa_id', 'site', 'horizon_classification1', 'horizon_classification2',
                                           'depth', 'methodological_notes')}),
...
class CollectionSiteAdmin(admin.ModelAdmin):
    ...
    class CollectionForm(forms.ModelForm):
        class Meta:
            ...
            widgets = {
                'location_name': AutosizedTextarea(attrs={'class': 'input-large',
                                                          'style': 'width:95%'}),
                'debug_note': AutosizedTextarea(attrs={'class': 'input-large',
                                                       'style': 'width:95%'}),
                'vegetation_dominant_trees': AutosizedTextarea(attrs={'class': 'input-large',
                                                                      'style': 'width:95%'}),
                'slope': EnclosedInput(append='%'),
                'fire_history': AutosizedTextarea(attrs={'class': 'input-large',
                                                         'style': 'width:95%'}),
                ...
                'agrochemical_additions': AutosizedTextarea(attrs={'class': 'input-large',
                                                                   'style': 'width:95%'}),
                'other_comments': AutosizedTextarea(attrs={'class': 'input-large',
                                                           'style': 'width:95%'}),
            }

    form = CollectionForm
    inlines = (SampleContextInline, )
    suit_form_tabs = (('description', 'Location Description'), ('context', 'Context'), ('landuse', 'Land Use'),
                      ('vegetation', 'Vegetation'), ('fire', 'Fire'), ('samples', 'Collection Samples'),
                      ('notes', 'Notes'))
    ...
    fieldsets = [
        ...
        (None,  # 'Fire',
         {'classes': ('suit-tab suit-tab-fire', ),
          ...
          'fields': ('fire_history', 'fire_intensity')}),
        (None,  # 'Notes',
...
class ChemicalAnalysisAdmin(admin.ModelAdmin):
    ...
    suit_form_tabs = (('structure', 'Soil Structure'),
                      ('chemical', 'Chemical'), )
